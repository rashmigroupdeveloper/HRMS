...
- [x] P0-T33 — Directory + profile UI shell (05 §4.2) — **done 11 Jul 2026**: `GET /api/employees` + `GET /api/employees/{ecode}` (`employee.read`, statutory masking); frontend `/people` + `/people/:ecode` tab shell (Compensation gated on `canViewCompensation`)
