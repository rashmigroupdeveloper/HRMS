...
| D3 | Coverage | **Employees on rolls first** (white collar, blue collar, trainees, consultants). The live EMS master (doc 11 §0) shows **14 legal entities / 1,066 employees**, not 6 — the primary India entities **RML (667), RGH (174), RDL (96), RPL (57)** run the full India-payroll engine, alongside smaller India entities (eHoome/Koove/Rashmi Rare Earth — confirm which actually payroll here). **Five foreign entities are in the master but OUT of India statutory-payroll scope** — RPF (Dubai FZCO), Reach Mining (Tanzania), Rashmi Metaliks UK, Rashmi Metaliks Bahrain — no PF/ESIC/PT/TDS; handled separately (manual/local-WPS) until a foreign-payroll module is scoped. The company master must **dedupe** on import (e.g. "Rashmi Metalix Ltd" ×6 is a misspelling of RML). Contract-worker module deferred (schema reserves `employment_category='contract'`). |
